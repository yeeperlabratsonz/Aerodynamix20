...
Bundled Media Player tracks should be represented as metadata-backed items with direct served audio URLs, rather than downloading every large audio file into browser memory during page initialization. Group the bundled catalog into the named starter pack and load audio on selection.
...
**Why:** Large FLAC downloads made page entry slow and sequential loading could fail independently; the visible overlay was rendered before the asynchronous unlock check completed.
